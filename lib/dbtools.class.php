  ...
  class dbTools
  {
      ...
      public static function doBackup($gzip = false)
      {
          if (!($sql = self::fetch())) {
              ...
              $data['sbackup'] = basename($fname);
			  App::get("Db")->update(Core::sTable, $data, array('id' => 1));
          ...
          foreach ($lines as $line_num => $line) {
              if (substr($line, 0, 2) != '--' && $line != '') {
                  ...
                  if (substr(trim($line), -1, 1) == ';') {
                      if (!App::get("Db")->pdoQuery($templine)) {
      ...
      private static function getTables()
      {
		  ...
		  $_oSTH = App::get("Db")->prepare('SHOW TABLES');
		  $_oSTH->execute();
      ...
      private static function dumpTable($table)
      {
		  ...
		  $_oSTH = App::get("Db")->prepare('SHOW CREATE TABLE ' . $table);
	  ...
	  public static function insert($table)
	  {
		  ...
		  $output = '';
		  if (!$query = App::get("Db")->select($table)->results()) {
			  return false;
		  }
      ...
      public static function fetch()
      {
          ...
          $dump .= '-- @package ' . App::get("Core")->wojon . ' v.' . App::get("Core")->wojov . self::nl;
          ...
          $dump .= '-- MySQL version: ' . App::get("Db")->getAttribute(PDO::ATTR_SERVER_VERSION) . self::nl;
