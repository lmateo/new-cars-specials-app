 ...
 */
  ...
  class wSpecials
  {
      ...
      {
      	$active = $status ? 'AND ws.modified_ws >= DATE(NOW() - INTERVAL 2 MONTH)' : 'AND ws.active = 0';
          ...
          if (isset($_GET['letter']) and (isset($_POST['fromdate_submit']) && $_POST['fromdate_submit'] <> "" || isset($from) && $from != '')) {
              ...
              $counter = self::$db->count(false, false, "SELECT COUNT(*) FROM `" . self::w_sTable . "` AS ws WHERE modelname_ws REGEXP '^" . $letter . "' $active LIMIT 1");
          	 ...
          	 $active = $status ? 'WHERE ws.modified_ws >= DATE(NOW() - INTERVAL 2 MONTH)' : 'WHERE ws.active = 0 AND ';
     ...
     /**
      ...
      public function getWebspecialsbystore($from = false, $page, $status = true, $id = false )
      {
      	...
      	if (isset($_GET['letter']) and (isset($_POST['fromdate_submit']) && $_POST['fromdate_submit'] <> "" || isset($from) && $from != '')) {
      		...
      		$active = $status ? 'AND ws.active = 1' : 'WHERE ws.active = 0 AND ';
      ...
      public function processWebspecials()
      {
	          ...
	          $validate->addRule('custom_save_up_to_text','string', true, 0, 10, Lang::$word->WSP_SAVE_UP_TO_TEXT);
          ...
          if (empty(Message::$msgs)) {
              ...
              $data = array(
              		...
              		'custom_save_up_to_text' => $validate->safe->custom_save_up_to_text,
      ...
      public function processWebspecialsDubs()
      {
      	...
      	if (empty(Message::$msgs)) {
      		...
      		$data = array(
      				...
      				'custom_save_up_to_text' => $wbsp->custom_save_up_to_text,
