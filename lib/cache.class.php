<?php
  ...
  class Cache
  {
       ...
       * @return
       */
      public static function cssCache($source)
      {

          $target = THEME . '/cache/';
          $last_change = self::lastChange($source);
          $temp = $target . self::prefix . 'main' . self::suffix;

          if (!file_exists($temp) || $last_change > filemtime($temp)) {
              if (!self::writeCssCache($source, $temp)) {
       ...
       * @return
       */
      protected static function lastChange($files)
      {
       ...
       * @return
       */
      protected static function writeCssCache($files, $target)
      {
